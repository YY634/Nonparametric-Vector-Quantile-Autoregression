#################### This R script will produce the lower right panel (d) in Figure 7 of the main text. #####################
...
load("./real_data_analysis/preprocessed_data/input.RData");   rm(CN_AD, FTD, CN_FTD) 
d <- 2

########## estimate the center-outward quantiles and medians of the stationary distribution (central panel)
samp <- matrix(0, 0, d)
for (i in 1:length(AD)){  tmp <- AD[[i]]; tmp <- tmp[1000:2000,];  samp <- rbind(samp, tmp) }
rm(tmp)
...
#save(stationary, file = "./real_data_analysis/AD_stationary.RData")
...
rm(tmp)
load("./real_data_analysis/preprocessed_data/AD_conds.RData") 
prediction <- Cond_Quantile_predict(AD_conds, h, qgridn, X, k_S, k_R, d=2)
#save(prediction, file = "./real_data_analysis/AD_predictive.RData")
...
for (i in 1:9){
  if (i < 5){
    ...
    points(medi[1],medi[2], col = "darkred", pch = 16, cex=1)
    for (k in 1:8){ points(AD_conds[k,1], AD_conds[k,2], col = "seagreen3", pch = 17, cex=1.2) }
  }
}
