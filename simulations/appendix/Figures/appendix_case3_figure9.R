...
d <- 2; T <- 4000000; burn <- 10000; samp <- matrix(0, T+1, d)
set.seed(730) 
...
samp[T+1,] <- X2
pairwisedist <- dist(samp[1:20000,]);  h <- 0.1*sum(pairwisedist)/length(pairwisedist) 
tims <- c(781, 1830, 2855, 3849, 4877, 5871, 6826, 7860, 8762, 9754)

X1 <- samp[1:20001,];  k_S <- 120; k_R <- 100; qgridn <- 180   
Upper_left <- Cond_Quantile(tims, h, qgridn, X1, k_S, k_R, d=2)
rm(X1)

X2 <- samp[1:40001,];  k_S <- 120; k_R <- 100; qgridn <- 180 
Upper_right <- Cond_Quantile(tims, h, qgridn, X2, k_S, k_R, d=2)
rm(X2)

X3 <- samp[1:400001,];  k_S <- 120; k_R <- 100; qgridn <- 180 
Lower_left <- Cond_Quantile(tims, h, qgridn, X3, k_S, k_R, d=2)
rm(X3)

k_S <- 120; k_R <- 100; qgridn <- 180 
theoretic <- Cond_Quantile(tims, h, qgridn, samp, k_S, k_R, d=2)

save(Upper_left, Upper_right, Lower_left, theoretic, file = "./simulations/appendix/Figures/app_case3_fig9.RData")
...
load("./simulations/appendix/Figures/app_case3_view.RData")
load("./simulations/appendix/Figures/app_case3_plot.RData")
...
rglwidget() 
snapshot3d("./simulations/appendix/Figures/case3_upper_left.png", width = 2000, height = 2000) 
...
grid3d_faces(nx = 7, ny = 4, nz = 4, x_plane = "min", y_plane = "max", z_plane = "min", col = "gray", lwd = 1)
rglwidget() 
snapshot3d("./simulations/appendix/Figures/case3_upper_right.png", width = 2000, height = 2000) 
...
grid3d_faces(nx = 7, ny = 4, nz = 4, x_plane = "min", y_plane = "max", z_plane = "min", col = "gray", lwd = 1)
rglwidget() 
snapshot3d("./simulations/appendix/Figures/case3_lower_left.png", width = 2000, height = 2000) 
...
grid3d_faces(nx = 7, ny = 4, nz = 4, x_plane = "min", y_plane = "max", z_plane = "min", col = "gray", lwd = 1)
rglwidget() 
snapshot3d("./simulations/appendix/Figures/case3_lower_right.png", width = 2000, height = 2000) 
