...
rm(FTD, AD, CN_AD, dba_AD, dba_CNAD, dba_FTD, dba_CNFTD)
d <- 2

########## estimate the center-outward quantiles and medians of the stationary distribution (central panel)
samp <- matrix(0, 0, d)
for (i in 1:length(CN_FTD)){  tmp <- CN_FTD[[i]]; tmp <- tmp[1000:2000,];  samp <- rbind(samp, tmp) }
rm(tmp)
...
#save(stationary, file = "./real_data_analysis/CNFTD_stationary.RData")
...
rm(pairwisedist)
load("./real_data_analysis/CNFTD_conds.RData") 
prediction <- Cond_Quantile_predict(CNFTD_conds, h, qgridn, X, k_S, k_R, d=2)
#save(prediction, file = "./real_data_analysis/CNFTD_predictive.RData")
...
for (i in 1:9){
  if (i < 5){
    ...
    points(medi[1],medi[2], col = "darkred", pch = 16, cex=1)
    for (k in 1:8){ points(CNFTD_conds[k,1], CNFTD_conds[k,2], col = "seagreen3", pch = 17, cex=1.2) }
  }
}
