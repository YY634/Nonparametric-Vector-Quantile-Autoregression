###################### This R script will produce the lower left panel (c) in Figure 7 of the main text. #####################
...
load("./real_data_analysis/preprocessed_data/input.RData");   rm(FTD, CN_FTD, AD) 
d <- 2

########## estimate the center-outward quantiles and medians of the stationary distribution (central panel)
samp <- matrix(0, 0, d)
for (i in 1:length(CN_AD)){  tmp <- CN_AD[[i]]; tmp <- tmp[1000:2000,];  samp <- rbind(samp, tmp) }
rm(tmp)
...
#save(stationary, file = "./real_data_analysis/CNAD_stationary.RData")
...
h <- 0.1;  k_S <- 100; k_R <- 100; qgridn <- 120 
X <- matrix(0, 0, d)
for (i in 1:length(CN_AD)){  tmp <- CN_AD[[i]]; L <- min(dim(tmp)[1], 100000); tmp <- tmp[1:L,];  X <- rbind(X, tmp) }
rm(tmp)
load("./real_data_analysis/preprocessed_data/CNAD_conds.RData") 
prediction <- Cond_Quantile_predict(CNAD_conds, h, qgridn, X, k_S, k_R, d=2)
#save(prediction, file = "./real_data_analysis/CNAD_predictive.RData")
...
for (i in 1:9){
  if (i < 5){
    ...
    points(medi[1],medi[2], col = "darkred", pch = 16, cex=1)
    for (k in 1:8){ points(CNAD_conds[k,1], CNAD_conds[k,2], col = "seagreen3", pch = 17, cex=1.2) }
  }
}
