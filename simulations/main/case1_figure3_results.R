...
d <- 2; T <- 44800; burn <- 800000; samp <- matrix(0, T+1, d)  
set.seed(810) 
...
################## estimating the quantiles and centers of the stationary distribution (the center panel in Figure 3) #################

#### estimating the stationary mean and covariance (quantiles) of a classical VAR model 
library(vars) 
...
save(mu_hat, Sigma_hat, file = "./simulations/main/case1_VAR_stationary.RData")
...
select_ind <- seq(2, T+1, by =2)
X <- samp[select_ind,]
k_S <- 160; k_R <- 140; n <- k_S*k_R;  qgridn <- 180  
...
save(stationary, file = "./simulations/main/case1_stationary.RData")




############### estimating the one-step ahead predictive quantiles and centers conditional on different current values (the surrounding panels in Figure 3)  #################

d <- 2; T <- 800000; burn <- 10000; samp <- matrix(0, T+1, d)  
set.seed(810) 
X1 <- gen_centered_SN(1, 6, 2)
for (t in 1:(burn+T)){
  c1 <- (X1[1] + X1[2])/3 -1;  c2 <- sqrt( (5 + sum(X1^2))/4 );  v <- sqrt(sum(X1^2))  
...
samp[T+1,] <- X2;   rm(X1, X2, c1, c2, v, inn, t, burn)
...
save(VAR_centers, VAR_SIGMs, file = "./simulations/main/case1_VAR_predictive.RData")
...
pairwisedist <- dist(samp[1:20000,]);  h <- 0.4*sum(pairwisedist)/length(pairwisedist)
k_S <- 100; k_R <- 100; qgridn <- 120 
#load("./simulations/main/case1_conds.RData")
prediction <- Cond_Quantile_predict(conds, h, qgridn, samp, k_S, k_R, d=2)
save(prediction, file = "./simulations/main/case1_predictive.RData")
